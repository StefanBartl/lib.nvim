...
## 15. Dashboard component & a generic jump-target concept

Moved to its own file: [`kit-dashboard.md`](kit-dashboard.md) — a
speculative, not-yet-built follow-on (a persistent activatable list + a
generic file/url/func/dir jump-target union), analysis only, still open (see
`ROADMAP.md`'s "Open concepts" list).
