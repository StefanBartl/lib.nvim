...
## 15. Dashboard component & a generic jump-target concept (analysis, not yet built)
...
### 15.1 What already exists, and why it doesn't fit
...
### 15.2 The other missing half: a generic "target"
...
### 15.3 Sketch: `kit.dashboard`

Not `menu` extended in place — `menu` is deliberately cursor-anchored and
transient (§8's "action list at cursor"); a dashboard is a **named, reusable,
...
- **Row → target mapping** is the caller's job, same as `chooser`'s rich
  ...
  becomes optional and the dashboard's own `activate()` dispatch (§15.2) runs
  by default — the "smart" default the task asked for, without forcing every
  caller to adopt it.
...
- **Persistence**: reuses `open_named_scratch`'s find-or-create-by-name
  ...
  no new window primitive, same "reuse, don't reimplement" principle as
  the rest of this doc (§2).

### 15.4 Migration path for `status_view.lua`
...
1. **Name.** `kit.dashboard` reads clearly next to `kit.picker`/`kit.menu`,
   ...
   trying to be — worth a bikeshed pass before committing to the name (see
   §14.1's precedent for `ui.kit` itself).
2. **Does `Lib.UI.Kit.Target` belong in `ui.kit` at all**, or in a lower
   ...
   *and* non-kit callers (`menu`, a future picker, plain keymaps) can share
   ...
   table in §15.2 has no float/buffer/theme dependency at all.
