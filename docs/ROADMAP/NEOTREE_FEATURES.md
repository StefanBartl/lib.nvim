...
- **Status:** already consumed by filetree.nvim · candidate (overlaps an
  ...
  counterpart yet, would need to be added first) · informational finding.
...
| Host env snapshot (`is_windows`/`home`/`pathsep`/`repo_base`) | `lib.nvim.system` | infra | newer/broader than `cross`; worth comparing before consolidating a port |
