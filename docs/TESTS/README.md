...
| `telemetry_spec.lua` | `lib.nvim.telemetry`: namespace sanitization, counting + exact identity restore on `stop()`, multi-return/vararg passthrough, `only`/`except`/`filter` scoping, `wrap_fn`, `wrap_loaded` (subtree discovery, dot-boundary prefix match, `module_only`/`module_except`/`module_filter`, prefix-relative keys, idempotent re-registration), predicate-form `profile_args`, the shared wrap layer (two instances, no double counting, no restore-ordering trap), argument fingerprinting + bounded cardinality + the memoization hint, timing/error counting, recursion (`outermost_only`), coverage, merge-on-write persistence, day windows + retention pruning, the lifecycle reminder, persistent per-namespace disable/enable (`toggle.lua`, dir-isolated), `:LibTelemetry` per-namespace start/stop/reset/disable/enable + completion narrowing, and `lib.strategies.control` key enumeration. |
