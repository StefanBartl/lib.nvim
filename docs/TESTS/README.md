...
| `telemetry_spec.lua` | `lib.nvim.telemetry`: namespace sanitization, counting + exact identity restore on `stop()`, multi-return/vararg passthrough, `only`/`except`/`filter` scoping, `wrap_fn`, `wrap_loaded` (subtree discovery, dot-boundary prefix match, `module_only`/`module_except`/`module_filter`, prefix-relative keys, idempotent re-registration, automatic module-id resolution), a plain `wrap()`'s key resolving only via an explicit `opts.module_id`, predicate-form `profile_args`, the shared wrap layer (two instances, no double counting, no restore-ordering trap), argument fingerprinting + bounded cardinality + the memoization hint, timing/error counting, recursion (`outermost_only`), coverage, merge-on-write persistence, `reset()` re-populating the module-id map for still-wrapped targets, day windows + retention pruning, the lifecycle reminder, reading a namespace off disk with no live instance (`store.load_readonly`/`telemetry.load`, incl. the nil-vs-empty distinction), persistent per-namespace disable/enable (`toggle.lua`, dir-isolated), `:LibTelemetry` per-namespace start/stop/reset/disable/enable + completion narrowing, `lib.strategies.control` key enumeration, `report.markdown()`/`markdown_all()` rendering (table rows, argument-profile subsections, the empty case, combined-document heading demotion), `report_file.lua` path resolution + writes + the `report_file = true` flush-time wiring, `report_style` resolution (`kit`/`mdview`/`file`/`auto`, deterministically falling back off-rtp), `telemetry.setup({ report_style = ... })`, `:LibTelemetry export`'s `.md`-vs-JSON format inference, `:LibTelemetry open [ns]` across all three styles plus completion, and `Options.info` (report metadata: carried into `report()`/`lines()`/`markdown()`, absent-when-empty, persistence, last-write-wins-wholesale merge on flush, and `reset()` keeping it available immediately). |
