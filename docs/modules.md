...
| [`lib.nvim.docmap`](../lua/lib/nvim/docmap/README.md) | generated module map: scans the annotated tree (opt-in LuaLS enrichment for type-reference edges), checks it for documentation drift, renders HTML (Tree + Hierarchy tabs)/Markdown/Mermaid (`:LibMap`), plus `install()`/`uninstall()` for a live in-memory handle another plugin's code can subscribe to |
